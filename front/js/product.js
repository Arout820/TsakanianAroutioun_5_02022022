                            ...
                            if (product["id"] == value._id){
                                if (product["color"] == storedProduct.color){
                                    if (product["quantity"] + productQuantityInt < 101){
                                        ...
                                        localStorage.setItem("product", JSON.stringify(basket));
                                        alert(`+ ${productQuantityInt} Vous avez ${product["quantity"]} / 100 quantités de ce produit dans le panier.`);
                                        test = false;
                                        break;
                                    } else {
                                        alert(`Vous avez déjà ${product["quantity"]} / 100 quantités de ce produit dans le panier. Vous ne pouvez pas dépasser 100 produits similaires merci.`);
                                        test = false;
                                        break;
                                    }
                                }
